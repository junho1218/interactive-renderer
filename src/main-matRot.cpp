...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Cam setting
        glm::vec3 tempCamPos = glm::vec3(rotation * glm::vec4(glm::vec3(0.0f, 0.0f, radius), 1.0f));
        glm::vec3 tempCamUp = glm::normalize(glm::vec3(rotation * glm::vec4(camUp, 0.0f)));
...
{ 
    if (clicking) { 
        ...
        float yaw = float(xpos - lastCursorX) * - sensitivity; 
        float pitch = float(ypos - lastCursorY) * - sensitivity;
